...
cmd:option('-lap_weights', '100')
cmd:option('-lap_layers', '2')
cmd:option('-lap_nobp', false)
...
local function main(params)
  ...
  local lap_layers = params.lap_layers:split(",")
  for i = 1, #lap_layers do
    lap_layers[i] = tonumber(lap_layers[i])
  end
  local lap_weights = params.lap_weights:split(",")
  for i = 1, #lap_weights do
    lap_weights[i] = tonumber(lap_weights[i])
  end
  assert( #lap_layers == #lap_weights )
  
  -- Set up the network, inserting style and content loss modules
  local content_losses, style_losses, lap_losses = {}, {}, {}
...
--[[
  local function lapofGauss(size, sigma)
    local center = 0.5 * size + 0.5
    local logauss = torch.Tensor(size,size)
    for i=1,size do
       for j=1,size do
           xsq = math.pow((i-center)/sigma,2)/2
           ysq = math.pow((j-center)/sigma,2)/2
           derivCoef = 1 - (xsq + ysq)
           logauss[i][j] = derivCoef * math.exp(-(xsq + ysq))
       end
    end
    logauss = logauss -  logauss:sum() / (size*size)
    logauss = logauss / logauss[{math.floor(center),math.floor(center)}]
    return logauss
  end

  local ks = 5
  logauss = lapofGauss(ks,0.5)
  lapW = torch.Tensor(1,3,ks,ks)
  for i = 1, 3 do
    lapW[{1,i,{},{}}] = logauss
  end  
...
--]]
  ...
  -- conv weight shape: out_channels, in_channels, kernel_size[0], kernel_size[1]
  local ks = 3
  laplacian = torch.Tensor(3,3):zero()
  laplacian[{1,2}] =        -1.0
  laplacian[{2,1}] = -1.0
  laplacian[{2,2}] =         4.0
  laplacian[{2,3}] =               -1.0
  laplacian[{3,2}] =        -1.0
  lapW = torch.Tensor(1,3,3,3)
  for i = 1, 3 do
    lapW[{1,i,{},{}}] = laplacian
  end
  
  stride = math.floor(ks/2)  
  for i = 1, #lap_layers do
    local netlap = nn.Sequential()
    local ps = math.pow(2, lap_layers[i])
    local avg_pool_layer = nn.SpatialAveragePooling(ps, ps, ps, ps):type(dtype)
    netlap:add(avg_pool_layer)
    local conv_layer = nn.SpatialConvolution(3,1,ks,ks,stride,stride):type(dtype)
    conv_layer.weight = lapW
    netlap:add(conv_layer)
    local norm = params.normalize_gradients
    local loss_module = nn.ContentLoss(lap_weights[i], norm):type(dtype)
    netlap:add(loss_module)
    table.insert(content_losses, loss_module)
    table.insert(lap_losses, loss_module)
    net:add(netlap)
  end  
  
  -- print(string.format('content losses: %d', #content_losses))
  ...
  if params.lap_nobp then
      for i = 1, #lap_losses do
        lap_losses[i].strength_bp = 0
      end
  end
  ...
  local function maybe_print(t, loss)
    local verbose = params.print_iter > 0 and 
            (t % params.print_iter == 0 or t == params.num_iterations - 1)
    if verbose then
...
function ContentLoss:__init(strength, normalize)
  ...
  -- strength when doing BP. used to disable lap_losses when -lap_nobp is specified
  self.strength_bp = strength
...
function ContentLoss:updateGradInput(input, gradOutput)
  if self.mode == 'loss' then
    ...
    self.gradInput:mul(self.strength_bp)
